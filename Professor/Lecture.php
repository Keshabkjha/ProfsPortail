...
if (strlen($_SESSION['trmsuid']==0)) {
    ...
    $sql = "SELECT courseName, lectureTime, roomNumber FROM tbllectureschedule WHERE teacherId = :teacherId AND lectureDay = :dayOfWeek";
            ...
            <div class="col-sm-6 col-lg-12">
                <div class="card text-white bg-flat-color-4">
                    <div class="card-body pb-0">
                        ...
                        <h4 class="text-light">Today's Lecture Schedule</h4>
                        <?php if (count($schedule) > 0): ?>
                            <ul>
                                <?php foreach ($schedule as $lecture): ?>
                                    <li><?php echo htmlentities($lecture->courseName); ?> at <?php echo htmlentities($lecture->lectureTime); ?> in <?php echo htmlentities($lecture->roomNumber); ?></li>
                                <?php endforeach; ?>
                            </ul>
                        <?php else: ?>
                            <p class="text-light">No lectures scheduled for today.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
